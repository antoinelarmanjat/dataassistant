...
Type anything to get started — for example, try **"Show my queries"** or **"Scan my datasets"**.`
  ...
  #conversations: Array<{id: string, title: string, last_active: number, default_dataset?: string}> = [];
  #sidebarOpen = true;
  ...
  async #deleteSession(sessionId: string, evt: Event) {
    evt.stopPropagation();
    try {
      const headers: Record<string, string> = {};
      if (this.#authUser?.credential) {
  ...
  #renderSessionList(): HTMLTemplateResult {
    const now = new Date();
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const yesterday = new Date(today.getTime() - 86400000);
    const lastWeek = new Date(today.getTime() - 7 * 86400000);

    const groups: Record<string, Array<{id: string, title: string, last_active: number, default_dataset?: string}>> = { today: [], yesterday: [], lastWeek: [], older: [] };
    ...
    const renderGroup = (label: string, items: Array<{id: string, title: string, last_active: number, default_dataset?: string}>) => {
      ...
      return html`
        ...
        ${items.map(c => html`
          <div class="session-item ${c.id === this.#currentSessionId ? 'active' : ''}"
            ...
            <button class="session-item-delete" @click=${(e: Event) => this.#deleteSession(c.id, e)} title="Delete">
              ...
              ${[
                ...
                <button
                  ...
                  @click=${async () => {
                    ...
                    this.#chatHistory = [...this.#chatHistory, {role: "user", content: body}];
                    const message: v0_8.Types.A2UIClientEventMessage = {request: body, session_id: this.#currentSessionId} as any;
                    await this.#sendAndProcessMessage(message);
                  }}
                >
